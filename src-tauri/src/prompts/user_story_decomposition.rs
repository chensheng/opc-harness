...
## 任务
请读取 @.opc-harness/PRD.md 获取 PRD 内容，将其拆分为符合 INVEST 原则的用户故事列表，并将结果保存到 @.opc-harness/US.md 文件中。

## ⚠️ 输出格式要求（非常重要）
**你必须在 US.md 文件中以 Markdown 表格格式输出用户故事，不要输出 JSON！**
...
**你必须将上述 Markdown 表格格式的用户故事列表写入到 `@.opc-harness/US.md` 文件中。**

### ⚠️ 覆盖写入规则（必须遵守）：
- **如果 US.md 文件不存在**：创建新文件并写入内容
- **如果 US.md 文件已存在**：**完全覆盖**原有内容，不要追加、不要合并
- **确保每次执行都是全新写入**，不包含任何历史内容

### 写入步骤：
1. 分析 PRD 内容，拆分为用户故事
2. 按照上述表格格式组织内容
3. **将整个 Markdown 表格内容写入 `@.opc-harness/US.md` 文件（覆盖模式）**
4. 确保文件格式正确，可以被正确解析

### ❌ 禁止的行为：
- **禁止输出 JSON 格式**
- **禁止在 US.md 中写入代码块标记（如 ```json 或 ```）**
- **禁止添加额外的解释说明文字**
- **禁止只输出而不写入文件**
- **禁止追加内容到现有文件（必须覆盖）**
- **禁止保留文件中的旧内容**
...
**请严格按照上述表格 format 输出到 US.md 文件，确保：**
1. 表格包含所有必需的列
2. 每行代表一个完整的用户故事
3. 验收标准用分号（；）分隔多条
4. 标签用逗号（，）分隔多个
...
7. **US.md 文件中只包含 Markdown 表格，没有其他内容**

**现在，请读取 @.opc-harness/PRD.md，将拆分后的用户故事以 Markdown 表格格式写入 @.opc-harness/US.md 文件：**"#;

/// 用户故事拆分提示词模板（适用于非 CodeFree 厂商，PRD 内容直接嵌入）
const USER_STORY_DECOMPOSITION_TEMPLATE_EMBEDDED: &str = r#"# 系统指令

你是一位经验丰富的敏捷开发专家和产品经理，擅长将产品需求拆分为符合 INVEST 原则的用户故事。

## 任务
请分析以下 PRD（产品需求文档），将其拆分为符合 INVEST 原则的用户故事列表，并以 Markdown 表格格式输出。
