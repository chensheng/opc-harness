...
async fn find_codefree_executable() -> Result<CodeFreeCommand, AIError> {
    ...
    #[cfg(windows)]
    {
        if let Some(path) = find_via_where().await? {
            let is_batch =
                path.to_lowercase().ends_with(".cmd") || path.to_lowercase().ends_with(".bat");
...
impl AIProvider {
    /// CodeFree CLI 非流式聊天
    pub(super) async fn chat_codefree(
        &self,
        request: ChatRequest,
        ...
        log::info!(
            "Using CodeFree CLI: {}, use_shell: {}",
            cmd_config.executable,
            cmd_config.use_shell
        );
        ...
        if !stderr.is_empty() {
            log::warn!(
                "CodeFree stderr (first 500 chars): {}",
        ...
        if !stdout.is_empty() {
            ...
            match self.parse_codefree_json_response(&stdout) {
                Ok(text) => {
                    log::info!(
                        "✓ Successfully parsed JSON from stdout, content length: {}",
                        text.len()
                    );
                ...
                Err(e) => {
                    ...
                    if e.message.starts_with("CodeFree CLI 错误：") {
                        log::info!(
                            "✓ Extracted specific error from stdout: {}",
        ...
        if !stderr.is_empty() {
            ...
            if let Some(json_start) = stderr.rfind('[') {
                ...
                log::info!(
                    "Found potential JSON in stderr, length: {}",
                    json_content.len()
                );
                log::info!(
                    "JSON preview: {}",
                ...
                match self.parse_codefree_json_response(json_content) {
                    Ok(text) => {
                        log::info!(
                            "✓ Successfully parsed JSON from stderr, content length: {}",
                            text.len()
                        );
                        return Ok(ChatResponse {
                            content: text,
                    ...
                    Err(e) => {
                        ...
                        if e.message.starts_with("CodeFree CLI 错误：") {
                            log::info!(
                                "✓ Extracted specific error from stderr: {}",
                                e.message.chars().take(100).collect::<String>()
                            );
                            return Err(e);
                        }
    ...
    fn parse_codefree_json_response(&self, json_str: &str) -> Result<String, AIError> {
        ...
        // 尝试解析 JSON 数组
        let responses: Vec<Value> = serde_json::from_str(json_str).map_err(|e| AIError {
    ...
    fn parse_codefree_jsonl_line(&self, line: &str) -> Result<Option<String>, AIError> {
        ...
        let value: Value = match serde_json::from_str(line) {
            ...
            Err(_) => {
                ...
                // 直接返回该行作为文本内容
                log::debug!(
                    "Line is not valid JSON, treating as plain text (length: {})",
                    line.len()
                );
        ...
        F: FnMut(String) -> Result<(), AIError>,
    {
        ...
        log::info!(
            "Using CodeFree CLI for streaming: {}, use_shell: {}",
            cmd_config.executable,
            cmd_config.use_shell
        );
        ...
        let workspace_dir = if let Some(ref project_id) = request.project_id {
            ...
            if project_workspace.exists() {
                log::info!(
                    "[CodeFree] ✅ Switching to project workspace: {:?}",
                    project_workspace
                );
                ...
                if settings_json.exists() {
                    ...
                    log::warn!(
                        "[CodeFree] ⚠️ settings.json NOT found at: {:?}",
                        settings_json
                    );
                ...
                log::warn!(
                    "[CodeFree] ❌ Project workspace does not exist: {:?}",
                    project_workspace
                );
            ...
            log::warn!(
                "[CodeFree] ⚠️ No project_id provided, running without workspace directory!"
            );
        ...
        if !status.success() {
            ...
            log::info!(
                "[CodeFree Error Debug] stdout length: {} bytes, stderr length: {} bytes",
                stdout_content.len(),
                stderr_content.len()
            );
            ...
            if !stdout_content.is_empty() {
                log::info!(
                    "Attempting to parse error from stream output (length: {})",
                    stdout_content.len()
                );
                ...
                for line in stdout_content.lines() {
                    ...
                    if !trimmed.is_empty() {
                        if let Err(e) = self.parse_codefree_jsonl_line(trimmed) {
                            if e.message.starts_with("CodeFree CLI 错误：") {
                                log::info!(
                                    "✓ Extracted error message from stream: {}",
                                    e.message.chars().take(100).collect::<String>()
                                );
                                return Err(e);
                            }
                ...
                log::warn!(
                    "[CodeFree Error Debug] ⚠️ stdout is empty, cannot extract error details"
                );
            }
        }
