...
impl AIProvider {
    ...
    async fn validate_codefree_cli(&self) -> Result<bool, AIError> {
        ...
        let version_cmd = Command::new("codefree").arg("--version").output().await;
        ...
        match &version_cmd {
            Ok(output) => {
                if output.status.success() {
                    ...
                    log::info!(
                        "✓ CodeFree CLI verified successfully (direct), version: {}",
                        version
                    );
                    return Ok(true);
                } else {
                    ...
                    log::warn!(
                        "Direct execution failed with status: {}, stderr: {}",
                        output.status,
                        stderr
                    );
                }
        ...
        #[cfg(windows)]
        {
            ...
            let where_cmd = Command::new("where").arg("codefree").output().await;
                    ...
                    if paths.trim().is_empty() {
                        ...
                        for path in paths.lines() {
                            ...
                            if !path.is_empty() {
                                ...
                                let test_cmd = if path.to_lowercase().ends_with(".cmd")
        ...
        #[cfg(unix)]
        {
            ...
            let which_cmd = Command::new("which").arg("codefree").output().await;
            ...
            if let Ok(output) = which_cmd {
                if output.status.success() {
                    ...
                    let test_cmd = Command::new(&path).arg("--version").output().await;
                    ...
                    if let Ok(test_output) = test_cmd {
                        if test_output.status.success() {
                            let version = String::from_utf8_lossy(&test_output.stdout)
                                ...
                                .to_string();
                            log::info!(
                                "✓ CodeFree CLI verified successfully via full path, version: {}",
                                version
                            );
                            return Ok(true);
                        }
