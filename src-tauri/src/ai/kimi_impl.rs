...
impl AIProvider {
    ...
    pub(super) async fn chat_kimi(&self, request: ChatRequest) -> Result<ChatResponse, AIError> {
        ...
        let is_coding_model =
            request.model.starts_with("kimi-coding") || request.model == "kimi-code";

        if is_coding_model {
            info!(
                "Using Kimi Coding (Anthropic-compatible) API for model: {}",
                request.model
            );

            // 检查 API Key 格式
            let auth_header_value = self.get_auth_header().1;
            let api_key = auth_header_value
                .strip_prefix("Bearer ")
                .unwrap_or(&auth_header_value);
            if !api_key.starts_with("sk-kimi-") {
                warn!(
                    "Kimi Coding API Key 格式可能不正确。期望以 'sk-kimi-' 开头，实际：{}",
                    api_key
                );
            }
            ...
            info!(
                "Sending Kimi Coding chat request to: {} with model: {}",
                url, coding_model_name
            );
            ...
            info!(
                "Using standard Kimi (OpenAI-compatible) API for model: {}",
                request.model
            );
        ...
        F: FnMut(String) -> Result<(), AIError>,
    {
        ...
        if is_coding_model {
            info!(
                "Using Kimi Coding (Anthropic-compatible) streaming API for model: {}",
                request.model
            );
            ...
            info!(
                "Sending Kimi Coding stream request to: {} with model: {}",
                url, coding_model_name
            );
                        ...
                        match serde_json::from_str::<serde_json::Value>(data) {
                            Ok(event) => {
                                ...
                                if let Some(content_block) = event.get("content_block") {
                                    ...
                                    {
                                        info!(
                                            "Extracted text from content_block: {} chars",
                                            text.len()
                                        );
                                ...
                                if event.get("content_block").is_none()
            ...
            info!(
                "Using standard Kimi (OpenAI-compatible) streaming API for model: {}",
                request.model
            );
            // 标准 Kimi API - 添加 system prompt
            let mut messages = Vec::new();
