...
pub struct AgentProcessInfo {
    ...
    pub pid: Option<u32>,              // 进程 ID（CLI Agent）
    pub status: AgentStatus,           // 运行状态
    pub started_at: i64,               // 启动时间戳
    pub resource_usage: ResourceUsage, // 资源使用情况

    // ========== Native Agent 特有字段 ==========
    pub is_native: bool,                 // 是否为 Native Agent
    pub token_usage: Option<TokenUsage>, // Token 使用统计（Native Agent）
    pub tool_calls_count: usize,         // 工具调用次数（Native Agent）
    pub current_turn: usize,             // 当前对话轮数（Native Agent）
    pub max_turns: usize,                // 最大对话轮数（Native Agent）
}
...
/// Token 使用统计（Native Agent）
#[derive(Debug, Clone, Serialize, Deserialize, Default)]
pub struct TokenUsage {
    pub prompt_tokens: usize,     // Prompt Token 数量
    pub completion_tokens: usize, // Completion Token 数量
    pub total_tokens: usize,      // 总 Token 数量
}

/// Native Agent 详细统计信息
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct NativeAgentStats {
    pub agent_id: String,                // Agent ID
    pub status: AgentStatus,             // 当前状态
    pub started_at: i64,                 // 启动时间戳
    pub elapsed_secs: i64,               // 已运行时长（秒）
    pub token_usage: Option<TokenUsage>, // Token 使用统计
    pub tool_calls_count: usize,         // 工具调用次数
    pub current_turn: usize,             // 当前对话轮数
    pub max_turns: usize,                // 最大对话轮数
    pub turn_progress: f32,              // 对话进度百分比 (0-100)
}
