...
impl AgentLoop {
    ...
    pub async fn execute_once(&mut self, project_id: &str) -> Result<usize, String> {
        ...
        if active_sprint.is_none() {
            ...
            log::info!("");
            log::info!("╔═══════════════════════════════════════════════════════════╗");
            log::info!("║           📝 DEBUG MODE - Example Log Output             ║");
            log::info!("╚═══════════════════════════════════════════════════════════╝");
            log::info!("[AgentLoop] ℹ️  This is an example of what logs will look like when Agent Loop runs with data:");
            log::info!("");
            log::info!("[AgentLoop] 🔍 Step 1: Checking for active Sprint...");
            log::info!("[AgentLoop] ✓ Found active Sprint: 'Sprint 1' (ID: uuid-example)");
            log::info!("[AgentLoop]   📅 Date range: 2026-05-01 to 2026-05-31");
            log::info!("[AgentLoop] ⏱️  Sprint check completed in 12ms");
            log::info!("");
            log::info!("[AgentLoop] 🔍 Step 2: Loading pending user stories from Sprint...");
            log::info!("[AgentLoop] ✓ Found 2 pending story/ies to process");
            log::info!("[AgentLoop]   Story #1: US-001 - Implement login feature (Priority: P0, Status: approved)");
            log::info!("[AgentLoop]   Story #2: US-002 - Add dashboard (Priority: P1, Status: refined)");
            log::info!("[AgentLoop] ⏱️  Stories loaded in 8ms");
            log::info!("");
            log::info!("[AgentLoop] 🚀 Step 3: Attempting to lock and start Agents for each story...");
            log::info!("[AgentLoop]   ┌─ Processing story US-001: Implement login feature...");
            log::info!("[AgentLoop]   │  Agent ID: coding-1746111030-0");
            log::info!("[AgentLoop]   │  🔒 Successfully locked story US-001 (Priority: P0)");
            log::info!("[AgentLoop]   │  🚀 Step 4: Starting Coding Agent for story US-001...");
            log::info!("[AgentLoop]   │  Creating worktree for agent coding-1746111030-0...");
            log::info!("[AgentLoop]   │  ✓ Worktree created at: C:\\...\\workspaces\\project\\.worktrees\\agent-coding-1746111030-0");
            log::info!("[AgentLoop]   │  🚀 Spawning AI CLI process in worktree...");
            log::info!("[Daemon] Spawning coding agent in worktree ... for story uuid-story with STDIO monitoring");
            log::info!("[AICLIInteraction] Starting to listen for agent coding-uuid with 1800s timeout");
            log::info!("[AgentLoop]   │  ✓ Successfully spawned coding agent");
            log::info!("[AgentLoop]   │  📝 Agent will work in isolated Worktree environment");
            log::info!("[AgentLoop]   └─ Agent started in 1.5s");
            log::info!("");
            log::info!("[AICLI:uuid-agent] STDOUT: Thinking about the problem...");
            log::info!("[AICLI:uuid-agent] STDOUT: Generating code structure...");
            log::info!("[AgentLoop:uuid-agent] 📝 Generated code for file: src/main.rs");
            log::info!("[AgentLoop:uuid-agent]   Content size: 2048 bytes");
            log::info!("[AgentLoop:uuid-agent] ✓ Successfully wrote generated code to: src/main.rs (in 15ms)");
            log::info!("[AICLI:uuid-agent] Agent process completed with status: ExitStatus(0)");
            log::info!("[AgentLoop:uuid-agent] 🏁 Task completed: SUCCESS");
            log::info!("[AgentLoop:uuid-agent] ⏱️  Total execution time: 45.3s");
            log::info!("[AgentLoop:uuid-agent] 📨 Total messages processed: 127");
            log::info!("");
            log::info!("[GitOps] ═══════════════════════════════════════════════");
            log::info!("[GitOps] Starting commit and push for worktree: C:\\...\\agent-uuid");
            log::info!("[GitOps] Step 1/5: Checking for changes (git status)...");
            log::info!("[GitOps] ✓ Detected 3 changed file(s)");
            log::info!("[GitOps] Step 2/5: Staging all changes (git add)...");
            log::info!("[GitOps] ✓ Successfully staged all changes");
            log::info!("[GitOps] Step 3/5: Creating commit (git commit)...");
            log::info!("[GitOps] ✓ Commit successful");
            log::info!("[GitOps] Step 4/5: Pushing to remote branch (git push)...");
            log::info!("[GitOps] ✓ Push successful");
            log::info!("[GitOps] ✅ All Git operations completed successfully!");
            log::info!("[GitOps] ⏱️  Total Git time: 2.7s");
            log::info!("");
            log::info!("[StoryStatus] 📊 Updating story uuid status to 'completed'");
            log::info!("[StoryStatus] ✓ Successfully updated 1 story(s) to completed (in 8ms)");
            log::info!("");
            log::info!("╔═══════════════════════════════════════════════════════════╗");
            ...
            log::info!("[AgentLoop]   • Total pending stories: 2");
            log::info!("[AgentLoop]   • Successfully started: 1 ✓");
            log::info!("[AgentLoop]   • Failed to start: 0 ✗");
            log::info!("[AgentLoop]   • Skipped (already locked): 1 ⊘");
            log::info!("[AgentLoop]   • Remaining pending: 1");
            log::info!("[AgentLoop] ⏱️  Performance:");
            log::info!("[AgentLoop]   • Total cycle time: 5.8s");
            log::info!("[AgentLoop]   • Agent startup phase: 5.5s");
            log::info!("[AgentLoop]   • Average per story: 1.8s");
            log::info!("");
            log::info!("💡 To see real logs, please:");
            log::info!("   1. Create a Sprint with status='active'");
            log::info!("   2. Add User Stories with status='approved' or 'refined'");
            log::info!("   3. Click 'Execute Once' button in the UI");
            log::info!("");
        ...
        if pending_stories.is_empty() {
            ...
            // 调试模式：输出 Agent 启动和执行的示例日志
            log::info!("");
            log::info!("╔═══════════════════════════════════════════════════════════╗");
            log::info!("║           📝 DEBUG MODE - Agent Execution Example        ║");
            log::info!("╚═══════════════════════════════════════════════════════════╝");
            log::info!("[AgentLoop] ℹ️  You have an active Sprint but no pending stories.");
            log::info!("[AgentLoop] ℹ️  Here's what will happen when you add stories:");
            log::info!("");
            log::info!("[AgentLoop]   ┌─ Processing story US-001: Implement login feature...");
            log::info!("[AgentLoop]   │  Agent ID: coding-1746111030-0");
            log::info!("[AgentLoop]   │  Attempting to acquire lock...");
            log::info!("[AgentLoop]   │  🔒 Successfully locked story US-001 (Priority: P0)");
            log::info!("[AgentLoop]   │  🚀 Step 4: Starting Coding Agent for story US-001...");
            log::info!("[AgentLoop]   │  Creating worktree for agent coding-1746111030-0 and story uuid-story");
            log::info!("[WorktreeManager] Git repository already initialized at C:\\...\\workspaces\\project");
            log::info!("[WorktreeManager] Creating worktree for agent coding-1746111030-0 from branch story-US-001");
            log::info!("[AgentLoop]   │  ✓ Worktree created at: C:\\...\\workspaces\\project\\.worktrees\\agent-coding-1746111030-0");
            log::info!("[AgentLoop]   │  Worktree creation time: 850ms");
            log::info!("[AgentLoop]   │  Message channel created (capacity: 100)");
            log::info!("[AgentLoop]   │  Spawning message handler task...");
            log::info!("[AgentLoop]   │  ✓ Message handler spawned in 2ms");
            log::info!("[AgentLoop]   │  🚀 Spawning AI CLI process in worktree...");
            log::info!("[Daemon] Spawning coding agent in worktree ... for story uuid-story with STDIO monitoring");
            log::info!("[Daemon] Retrieved story context for uuid-story: title='Implement login feature', acceptance_criteria_length=150");
            log::info!("[Daemon] Building CLI command for worktree with full context: kimi [...]");
            log::info!("[AICLIInteraction] Starting to listen for agent coding-uuid-agent with 1800s timeout");
            log::info!("[AgentLoop]   │  ✓ Successfully spawned coding agent in worktree with STDIO monitoring: coding-uuid-agent");
            log::info!("[AgentLoop]   │  ⏱️  Agent spawn time: 150ms");
            log::info!("[AgentLoop]   │  ⏱️  Total setup time: 1.2s");
            log::info!("[AgentLoop]   │  ✓ Successfully started Coding Agent #1 for story US-001");
            log::info!("[AgentLoop]   │  📝 Agent will work in isolated Worktree environment");
            log::info!("[AgentLoop]   └─ Agent started in 1.5s");
            log::info!("");
            log::info!("[AgentLoop:coding-uuid-agent] Message handler started");
            log::info!("[AICLI:uuid-agent] STDOUT: Thinking about the problem...");
            log::info!("[AgentLoop:uuid-agent] AI Output [msg #1]: Analyzing requirements...");
            log::info!("[AICLI:uuid-agent] STDOUT: Generating code structure...");
            log::info!("[AgentLoop:uuid-agent] AI Output [msg #2]: Creating file structure...");
            log::info!("[AgentLoop:uuid-agent] 📝 Generated code for file: src/main.rs");
            log::info!("[AgentLoop:uuid-agent]   Content size: 2048 bytes");
            log::info!("[AgentLoop:uuid-agent]   Acquiring file write lock...");
            log::info!("[AgentLoop:uuid-agent] ✓ Successfully wrote generated code to: src/main.rs (in 15ms)");
            log::info!("[AgentLoop:uuid-agent] 📝 Generated code for file: src/utils.rs");
            log::info!("[AgentLoop:uuid-agent]   Content size: 1024 bytes");
            log::info!("[AgentLoop:uuid-agent] ✓ Successfully wrote generated code to: src/utils.rs (in 10ms)");
            log::info!("[AICLI:uuid-agent] Agent process completed with status: ExitStatus(0)");
            log::info!("[AgentLoop:uuid-agent] 🏁 Task completed: SUCCESS - All files generated successfully");
            log::info!("[AgentLoop:uuid-agent] ⏱️  Total execution time: 45.3s");
            log::info!("[AgentLoop:uuid-agent] 📨 Total messages processed: 127");
            log::info!("[AgentLoop:uuid-agent] Message handler exited");
            log::info!("");
            log::info!("[AgentLoop:uuid-agent] 🔄 Starting Git operations...");
            log::info!("[GitOps] ═══════════════════════════════════════════════");
            log::info!("[GitOps] Starting commit and push for worktree: C:\\...\\agent-uuid-agent");
            log::info!("[GitOps] Story ID: uuid-story");
            log::info!("[GitOps] Step 1/5: Checking for changes (git status)...");
            log::info!("[GitOps] ✓ Detected 2 changed file(s)");
            log::info!("[GitOps] Changed files:");
            log::info!("[GitOps]  M src/main.rs");
            log::info!("[GitOps]  M src/utils.rs");
            log::info!("[GitOps] ⏱️  Status check completed in 25ms");
            log::info!("[GitOps] Step 2/5: Staging all changes (git add)...");
            log::info!("[GitOps] ✓ Successfully staged all changes");
            log::info!("[GitOps] ⏱️  Git add completed in 30ms");
            log::info!("[GitOps] Commit message: Auto-generated code for story uuid-story");
            log::info!("[GitOps] Step 3/5: Creating commit (git commit)...");
            log::info!("[GitOps] ✓ Commit successful");
            log::info!("[GitOps] Commit output: [story-uuid-story abc123] Auto-generated code for story uuid-story");
            log::info!("[GitOps] ⏱️  Git commit completed in 120ms");
            log::info!("[GitOps] Step 4/5: Pushing to remote branch 'story-uuid-story' (git push)...");
            log::info!("[GitOps] ✓ Push successful");
            log::info!("[GitOps] ⏱️  Git push completed in 2.5s");
            log::info!("[GitOps] ═══════════════════════════════════════════════");
            log::info!("[GitOps] ✅ All Git operations completed successfully!");
            log::info!("[GitOps] 📊 Summary:");
            log::info!("[GitOps]   • Changed files: 2");
            log::info!("[GitOps]   • Branch: story-uuid-story");
            log::info!("[GitOps]   • Commit message: Auto-generated code for story uuid-story");
            log::info!("[GitOps] ⏱️  Total Git time: 2.7s");
            log::info!("");
            log::info!("[AgentLoop:uuid-agent] ✓ Successfully committed and pushed changes: Auto-generated code for story uuid-story");
            log::info!("[AgentLoop:uuid-agent] ⏱️  Git operations completed in 2.7s");
            log::info!("[AgentLoop:uuid-agent] 📊 Updating story status to 'completed'...");
            log::info!("[StoryStatus] 📊 Updating story uuid-story status to 'completed'");
            log::info!("[StoryStatus]   Acquiring database connection...");
            log::info!("[StoryStatus]   ✓ Database connection acquired");
            log::info!("[StoryStatus]   Executing UPDATE query...");
            log::info!("[DB::complete_user_story] Completed story: uuid-story");
            log::info!("[StoryStatus] ✓ Successfully updated 1 story(s) to completed (in 8ms)");
            log::info!("[AgentLoop:uuid-agent] ✓ Successfully updated story status to completed (in 8ms)");
            log::info!("");
            log::info!("💡 To see real execution logs, please:");
            log::info!("   1. Go to 'User Stories' tab");
            log::info!("   2. Create a new User Story");
            log::info!("   3. Set status to 'approved' or 'refined'");
            log::info!("   4. Assign it to the current Sprint");
            log::info!("   5. Click 'Execute Once' button");
            log::info!("");
