...
use super::event_bus::{EventBusClient, EventMessage, EventType, SharedEventBus};
...
impl DecentralizedAgentNode {
    ...
    pub async fn new(
        ...
        log::info!(
            "[DecentralizedNode] Created node {} with config: {:?}",
            config.node_id,
            config
        );
    ...
    pub fn set_worktree_manager(&mut self, project_path: &str) {
        ...
        log::info!(
            "[DecentralizedNode:{}] Worktree manager initialized",
            self.config.node_id
        );
    }
    ...
    pub async fn start(&mut self) -> Result<(), String> {
        ...
        log::info!(
            "[DecentralizedNode:{}] Starting decentralized agent loop",
            self.config.node_id
        );
        ...
        tokio::spawn(async move {
            ...
            loop {
                match event_rx.recv().await {
                    Ok(message) => {
                        log::info!(
                            "[DecentralizedNode:{}] Received event: {}",
                            node_id,
                            message.event_type
                        );
                        ...
                        if message.event_type == "story.created" {
                            ...
                            {
                                log::error!(
                                    "[DecentralizedNode:{}] Failed to handle story.created: {}",
                                    node_id,
                                    e
                                );
                            }
                    ...
                    Err(tokio::sync::broadcast::error::RecvError::Lagged(n)) => {
                        log::warn!(
                            "[DecentralizedNode:{}] Lagged behind by {} messages",
                            node_id,
                            n
                        );
                    }
    ...
    async fn handle_story_created(
        ...
        log::info!(
            "[DecentralizedNode:{}] Evaluating story {} from project {}",
            node_id,
            story_id,
            project_id
        );
        ...
        if !Self::should_accept_story(node_id, max_concurrent).await {
            log::info!(
                "[DecentralizedNode:{}] Declining story {} (load too high or not capable)",
                node_id,
                story_id
            );
            return Ok(());
        ...
        if !lock_acquired {
            log::info!(
                "[DecentralizedNode:{}] Story {} already locked by another node",
                node_id,
                story_id
            );
            return Ok(());
        ...
        let worktree_path = if let Some(ref wt_manager) = worktree_manager {
            ...
            wt_manager
                .create_worktree(node_id, story_id, &branch_name)
                .await?
        } else {
        ...
        log::info!(
            "[DecentralizedNode:{}] Started agent {} for story {} at {}",
            node_id,
            agent_id,
            story_id,
            worktree_path
    ...
    async fn should_accept_story(node_id: &str, _max_concurrent: usize) -> bool {
        ...
        log::debug!(
            "[DecentralizedNode:{}] Decision: accept={}",
            node_id,
            accept
        );
    ...
    pub async fn stop(&mut self) -> Result<(), String> {
        ...
        log::info!(
            "[DecentralizedNode:{}] Stopping decentralized agent loop",
            self.config.node_id
        );
