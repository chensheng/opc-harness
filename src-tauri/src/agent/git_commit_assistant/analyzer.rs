...
use super::types::{ChangeInfo, CommitType, FileChangeType};
    ...
    pub fn categorize_changes(&self, changes: &[ChangeInfo]) -> CommitType {
        ...
        for change in changes {
            ...
            if file_lower.ends_with(".css")
                || file_lower.ends_with(".scss")
                || file_lower.ends_with(".less")
            {
                has_style = true;
            }

            // 检测修复相关关键字
            if change.file_path.contains("fix")
                || change
                    .diff_summary
                    .as_ref()
            ...
            // 检测新功能相关关键字
            if change.file_path.contains("feature")
                || change
                    .diff_summary
                    .as_ref()
                    .is_some_and(|s| s.contains("add") || s.contains("new"))
            {
                has_feature = true;
            }
        }
        ...
        if has_test
            && changes
                .iter()
                .all(|c| c.file_path.contains("test") || c.file_path.contains("spec"))
        {
            CommitType::Test
        } else if has_doc {
            CommitType::Docs
