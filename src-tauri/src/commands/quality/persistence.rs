use crate::commands::quality::types::{
    GetUserStoriesRequest, GetUserStoriesResponse, SaveUserStoriesRequest, SaveUserStoriesResponse,
    UserStory,
};
use crate::db;
...
pub async fn save_user_stories(
    ...
    println!(
        "[save_user_stories] Received request for project_id: {}, stories count: {}",
        request.project_id,
        request.user_stories.len()
    );
    ...
    let db_stories: Vec<DbUserStory> = request
        .user_stories
        .iter()
        .map(|story| {
            ...
            DbUserStory {
                ...
                acceptance_criteria: serde_json::to_string(&story.acceptance_criteria)
                ...
                labels: Some(
                    serde_json::to_string(&story.labels).unwrap_or_else(|_| "[]".to_string()),
                ),
                dependencies: story
                    .dependencies
                    .as_ref()
    ...
    println!(
        "[save_user_stories] Converted {} stories to DB format",
        db_stories.len()
    );
    ...
    match db::upsert_user_stories(&conn, &request.project_id, &db_stories) {
        Ok(_) => {
            println!(
                "[save_user_stories] Successfully saved {} stories to database for project {}",
                db_stories.len(),
                request.project_id
            );
        }
...
pub async fn get_user_stories(
    ...
    let user_stories: Vec<UserStory> = db_stories
        .iter()
        .map(|story| UserStory {
            id: story.id.clone(),
            story_number: story.story_number.clone(),
            ...
            benefit: story.benefit.clone(),
            description: story.description.clone(),
            acceptance_criteria: serde_json::from_str(&story.acceptance_criteria)
                .unwrap_or_default(),
            priority: story.priority.clone(),
