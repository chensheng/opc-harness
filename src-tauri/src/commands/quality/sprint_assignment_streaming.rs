...
"#,
            ...
            labels = if story.labels.is_empty() {
                "无".to_string()
            } else {
                story.labels.join(", ")
            },
...
pub async fn assign_stories_to_sprint_streaming(
    ...
    log::info!(
        "Starting streaming sprint story assignment (provider: {}, model: {})",
        request.provider,
        request.model
    );
    ...
    let user_message = if request.provider == "codefree" {
        ...
        if let Some(ref pid) = request.project_id {
            ...
            log::info!(
                "[assign_stories_to_sprint_streaming] 📁 Workspace path: {:?}",
                workspace_path
            );
            log::info!(
                "[assign_stories_to_sprint_streaming] 📁 Context directory: {:?}",
                context_dir
            );
            ...
            fs::create_dir_all(&context_dir).map_err(|e| {
                log::error!(
                    "[assign_stories_to_sprint_streaming] Failed to create context directory: {}",
                    e
                );
            ...
            let agents_content =
                crate::prompts::sprint_assignment::generate_sprint_assignment_system_prompt();

            fs::write(&agents_md_path, &agents_content).map_err(|e| {
                log::error!(
                    "[assign_stories_to_sprint_streaming] Failed to write AGENTS.md: {}",
                    e
                );
                format!("Failed to write AGENTS.md: {}", e)
            })?;
            log::info!(
                "[assign_stories_to_sprint_streaming] ✅ AGENTS.md written to: {:?}",
                agents_md_path
            );
            log::info!(
                "[assign_stories_to_sprint_streaming] 📝 AGENTS.md content length: {} bytes",
                agents_content.len()
            );
            ...
            fs::write(&us_md_path, &us_content).map_err(|e| {
                log::error!(
                    "[assign_stories_to_sprint_streaming] Failed to write US.md: {}",
                    e
                );
                format!("Failed to write US.md: {}", e)
            })?;
            log::info!(
                "[assign_stories_to_sprint_streaming] ✅ US.md written to: {:?}",
                us_md_path
            );
...
"#,
            ...
            fs::write(&sprint_info_md_path, &sprint_info_content).map_err(|e| {
                log::error!(
                    "[assign_stories_to_sprint_streaming] Failed to write SPRINT_INFO.md: {}",
                    e
                );
                format!("Failed to write SPRINT_INFO.md: {}", e)
            })?;
            log::info!(
                "[assign_stories_to_sprint_streaming] ✅ SPRINT_INFO.md written to: {:?}",
                sprint_info_md_path
            );

            // 构建简短的用户消息，通过 @ 引用文件，并将用户建议直接放入query中
            let mut message = String::from("请读取 @.opc-harness/AGENTS.md 了解任务规则，");
