...
pub fn parse_markdown_to_quality_prd(content: &str) -> QualityPRDDocument {
    let mut title = None;
    let mut overview = None;
...
pub fn parse_markdown_to_feasibility_prd(content: &str) -> FeasibilityPRDDocument {
    let mut title = None;
    let mut overview = None;
    let mut target_users = None;
    let mut core_features = None;
...
/// 解析列表项（支持 -, *, • 等标记）
pub fn parse_list_items(text: &str) -> Vec<String> {
