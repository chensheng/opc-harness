...
pub async fn check_git_status(path: String) -> Result<GitStatus, String> {
    ...
    // 检查是否是 Git 仓库
    let is_git_repo = tokio::fs::metadata(path_buf.join(".git"))
        .await
        .is_ok();
    ...
    log::info!("[check_git_status] Git repo: {}, branch: {:?}, commits: {:?}", 
               is_git_repo, branch, commit_count);
...
pub async fn init_git_repo(path: String, initial_branch: Option<String>) -> Result<bool, String> {
    ...
    log::info!("[init_git_repo] Git initialized with branch: {}", branch_arg);
...
async fn create_gitignore(project_path: &str) -> Result<(), String> {
    ...
    log::info!("[create_gitignore] Created .gitignore at {:?}", gitignore_path);
    Ok(())
}
