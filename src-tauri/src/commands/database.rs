...
/// 手动执行数据库迁移（一次性操作）
/// 
/// 此命令用于为现有数据库添加 sprint_id 字段支持
#[tauri::command]
pub fn run_database_migration(app_handle: tauri::AppHandle) -> Result<String, String> {
    println!("[Migration] Starting manual database migration...");
    
    let db_path = crate::utils::paths::get_database_path();
    println!("[Migration] Database path: {:?}", db_path);
    ...
    // 执行迁移（直接调用内部函数）
    match crate::db::database::migrate_add_sprint_id_support() {
        Ok(_) => {
            println!("[Migration] ✓ Migration completed successfully!");
            
            // 验证列是否存在
            let conn = crate::db::get_connection(&app_handle).map_err(|e| e.to_string())?;
            let column_exists = conn.query_row(
                "SELECT COUNT(*) FROM pragma_table_info('user_stories') WHERE name='sprint_id'",
                [],
                |row| row.get::<_, i64>(0)
            ).map_err(|e| format!("Failed to verify column: {}", e))?;
            
            if column_exists > 0 {
                println!("[Migration] ✓ Verified: sprint_id column exists in user_stories table");
                ...
                let index_exists = conn.query_row(
                    ...
                    [],
                    |row| row.get::<_, i64>(0)
                ).map_err(|e| format!("Failed to verify index: {}", e))?;
                
                if index_exists > 0 {
                    println!("[Migration] ✓ Verified: Index exists on sprint_id");
                    Ok("Migration successful! sprint_id column and index verified.".to_string())
                } else {
                    Err("Migration completed but index verification failed!".to_string())
                }
            } else {
                Err("Migration reported success but column not found!".to_string())
            }
        },
        Err(e) => {
            eprintln!("[Migration] ✗ Migration failed: {}", e);
