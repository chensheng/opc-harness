...
pub fn create_project(
    ...
    if let Err(e) = initialize_git_repository(&workspace_path.to_string_lossy()) {
        ...
        log::info!(
            "Project workspace created with Git initialized at: {:?} (ID: {})",
            workspace_path,
            project_id
        );
    }
...
fn initialize_git_repository(project_path: &str) -> Result<(), String> {
    ...
    log::info!(
        "[initialize_git_repository] Initializing Git at: {}",
        project_path
    );
    ...
    if !commit_output.status.success() {
        ...
        log::warn!(
            "[initialize_git_repository] Initial commit warning: {}",
            stderr
        );
    } else {
...
fn create_gitignore_file(project_path: &str) -> Result<(), String> {
    ...
    log::info!(
        "[create_gitignore_file] Created .gitignore at {:?}",
        gitignore_path
    );
    Ok(())
}
...
fn ensure_workspace_and_git(
    ...
    if !workspace_path.exists() {
        log::info!(
            "[ensure_workspace_and_git] Workspace directory missing, recreating: {:?}",
            workspace_path
        );
...
/// 获取单个 Issue
#[tauri::command]
pub fn get_issue_by_id(_app_handle: tauri::AppHandle, id: String) -> Result<Option<Issue>, String> {
    let conn = db::get_connection().map_err(|e| e.to_string())?;
...
pub fn get_sessions_by_project(
    ...
    let conn = db::get_connection().map_err(|e| {
        log::error!(
            "[get_sessions_by_project] Failed to get database connection: {}",
            e
        );
...
pub fn save_sprints(
    ...
    println!(
        "[save_sprints] Received request for project_id: {}, sprints count: {}",
        request.project_id,
        request.sprints.len()
    );
    ...
    match db::upsert_sprints(&conn, &request.project_id, &request.sprints) {
        Ok(_) => {
            println!(
                "[save_sprints] Successfully saved {} sprints to database for project {}",
                request.sprints.len(),
                request.project_id
            );
...
pub fn get_sprints_by_project(
    ...
    let conn = db::get_connection().map_err(|e| {
        eprintln!(
            "[get_sprints_by_project] Failed to get DB connection: {}",
            e
        );
        format!("Failed to get DB connection: {}", e)
    })?;
...
pub fn get_sprint_stories(
    ...
    println!(
        "[get_sprint_stories] Querying for sprint_id: {}",
        request.sprint_id
    );
...
mod tests {
    ...
    fn test_create_workspace_directory_with_uuid() {
        ...
        // 验证临时目录路径正确（防止路径穿越）
        assert!(
            workspaces_root.starts_with(&temp_dir),
        ...
        assert!(
            workspace_path.starts_with(&workspaces_root),
