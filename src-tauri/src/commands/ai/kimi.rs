...
pub async fn chat_kimi(request: ChatRequestPayload) -> Result<crate::ai::ChatResponse, String> {
    ...
    log::info!(
        "Kimi chat response received: {} chars",
...
pub async fn stream_chat_kimi(
    ...
    match provider.stream_chat(chat_request, on_chunk).await {
        Ok(final_content) => {
            ...
            log::info!(
                "Kimi streaming chat completed: {} chars",
                final_content.len()
            );
            Ok(final_content)
        }
...
pub async fn generate_personas_kimi(
    ...
    log::info!(
        "Generating user personas with Kimi for idea: {}",
        request.idea
    );
    ...
    log::info!(
        "User persona generated successfully with Kimi: {}",
        persona.name
    );
...
pub async fn generate_competitor_analysis_kimi(
    ...
    log::info!(
        "Generating competitor analysis with Kimi for idea: {}",
        request.idea
    );

    // 1. 构建竞品分析提示词（中国市场优化）
    let prompt = format!(
        r#"请为以下产品想法进行详细的竞品分析，重点关注中国市场：
