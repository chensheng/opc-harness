...
pub async fn generate_prd(request: GeneratePRDRequest) -> Result<PRDResponse, String> {
    ...
    let user_message = if request.provider == "codefree" {
        ...
        if let Some(ref pid) = request.project_id {
            ...
            log::info!(
                "[generate_prd] ✅ AGENTS.md successfully written to: {:?}",
                agents_md_path
            );
    ...
    let prd =
        parse_prd_from_markdown(&response.content).map_err(|e| format!("PRD 解析失败：{}", e))?;
...
pub async fn start_prd_stream(
    ...
    log::info!(
        "Starting streaming PRD generation for idea: {}",
        request.idea
    );
    ...
    log::info!(
        "[start_prd_stream] API Key length: {}",
        request.api_key.len()
    );
    ...
    // 调试：打印原始参数的类型信息
    log::info!(
        "[start_prd_stream] Project ID is_some: {}",
        ...
        if let Some(ref pid) = request.project_id {
            ...
            if !workspace_path.exists() {
                ...
                fs::create_dir_all(&workspace_path).map_err(|e| {
                    log::error!(
                        "[start_prd_stream] Failed to create workspace directory: {}",
                        e
                    );
            ...
            fs::create_dir_all(&context_dir).map_err(|e| {
                log::error!(
                    "[start_prd_stream] Failed to create context directory: {}",
                    e
                );
                format!("Failed to create context directory: {}", e)
            })?;
            ...
            log::info!(
                "[start_prd_stream] ✅ AGENTS.md successfully written to: {:?}",
                agents_md_path
            );
            log::info!(
                "[start_prd_stream] 📝 AGENTS.md content length: {} bytes",
                agents_content.len()
            );
            ...
            Ok(final_content) => {
                ...
                let prd_content = if provider_clone == "codefree" {
                    if let Some(ref pid) = project_id_clone {
                        ...
                        log::info!(
                            "[start_prd_stream] 📖 Reading generated PRD from: {:?}",
                            prd_md_path
                        );
                        ...
                        log::info!(
                            "[start_prd_stream] 🔄 Using streamed content, length: {} bytes",
                            final_content.len()
                        );
                        final_content
                    }
                ...
                log::info!(
                    "Streaming PRD generation completed, content length: {}",
                    prd_content.len()
                );
            }
            Err(e) => {
    ...
    // 立即返回 session_id
    Ok(StartPRDStreamResponse {
        session_id: session_id_for_return,
    })
}
