...
<body>
  ...
  describe('Content Display', () => {
    it('highlights added lines correctly', async () => {
      const addedLines = MOCK_FILE_DIFF.hunks[0].lines.filter(l => l.change_type === 'added')
    ...
    it('highlights removed lines correctly', async () => {
      const removedLines = MOCK_FILE_DIFF.hunks[0].lines.filter(l => l.change_type === 'removed')
    ...
    it('shows empty space for missing line numbers', async () => {
      const addedLine = MOCK_FILE_DIFF.hunks[0].lines.find(l => l.change_type === 'added')
      const removedLine = MOCK_FILE_DIFF.hunks[0].lines.find(l => l.change_type === 'removed')
