...
# 用法：.\.harness\scripts\harness-check.ps1
...
if ($Score -lt 100) {
    ...
    if ($Score -lt 80) {
        Write-Host "  1. Run 'npm run lint:fix' to auto-fix code style issues" -ForegroundColor Gray
    ...
    if ($Score -lt 60) {
        ...
        Write-Host "  6. Check if .harness/ directory structure is complete" -ForegroundColor Gray
    }
    Write-Host ""
}
