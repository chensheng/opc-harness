      ...
      <div className="max-w-6xl mx-auto">
                  ...
                  <div className="flex justify-between items-start">
                    <div className="flex-1">
                      ...
                      <div className="text-sm text-gray-600 dark:text-gray-400">
                        {worker.current_story_id ? (
                          <span>正在处理 Story: <strong>{worker.current_story_id}</strong></span>
                        ) : (
                          <span>空闲，等待新任务...</span>
                        )}
                      </div>
        ...
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow p-6 mt-6">
          ...
          <div className="space-y-3 text-sm text-gray-600 dark:text-gray-400">
            <div className="flex items-start gap-2">
              ...
              <span><strong>完全独立:</strong> 每个 Worker 拥有完整的 Agent Loop，不依赖中心调度器</span>
            </div>
            <div className="flex items-start gap-2">
              <span className="text-green-500">✅</span>
              <span><strong>数据库查询:</strong> Worker 定时查询数据库获取活跃 Sprint 和待处理的 User Stories</span>
            </div>
            <div className="flex items-start gap-2">
              <span className="text-green-500">✅</span>
              <span><strong>乐观锁:</strong> 多个 Worker 通过数据库乐观锁竞争领取同一个 Story</span>
            </div>
            <div className="flex items-start gap-2">
              <span className="text-green-500">✅</span>
              <span><strong>零外部依赖:</strong> 纯内存实现，无需 Redis 或 EventBus</span>
            </div>
            <div className="flex items-start gap-2">
              <span className="text-green-500">✅</span>
              <span><strong>自动状态更新:</strong> 任务完成后自动更新 Story 状态为 completed 或 failed</span>
            </div>
          </div>

          <div className="mt-6 p-4 bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg">
            <p className="text-sm text-blue-800 dark:text-blue-300">
              💡 <strong>提示:</strong> 启动多个 Worker 后，它们会独立查询数据库并竞争处理新创建的 Story。
              第一个成功获取乐观锁的 Worker 将执行任务，其他 Worker 会自动跳过该 Story 并继续等待下一个。
            </p>
          </div>
        </div>
        ...
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow p-6 mt-6">
          ...
          <div className="space-y-2 text-sm font-mono text-gray-700 dark:text-gray-300">
            ...
            <div>   └─ SELECT * FROM user_stories WHERE status IN ('draft', 'refined', 'approved')</div>
            <div>2. 尝试乐观锁锁定 Story</div>
            <div>   └─ UPDATE user_stories SET locked_by='worker-id' WHERE id=? AND (locked_by IS NULL OR locked_at &lt; NOW()-30min)</div>
