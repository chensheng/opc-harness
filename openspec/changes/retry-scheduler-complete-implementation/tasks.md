...
- [ ] 8.4 手动测试：创建一个失败的 Story，等待自动重试，验证端到端流程（需要运行时环境）
- [ ] 8.5 性能测试：模拟 100 个待重试 Story，验证扫描和触发的性能（已添加性能监控日志）

## 9. 文档更新

- [x] 9.1 在代码中添加必要的注释，解释关键逻辑和设计决策（已在实现中添加）
- [ ] 9.2 更新 `AGENT_CREATION.md` 或相关文档，说明重试调度器的工作原理（需要确认文档位置）
- [ ] 9.3 更新 README 或用户文档，说明如何配置重试参数（需要确认文档位置）
