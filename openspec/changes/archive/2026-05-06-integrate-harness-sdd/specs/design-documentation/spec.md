...
系统 SHALL 提供明确的决策矩阵,指导开发者在不同场景下是否需要更新 SDD 文档。

#### Scenario: 重大架构变更必须更新 SDD
- **WHEN** 变更涉及新增分层、改变数据流规则、修改核心架构模式
- **THEN** 开发者 MUST 更新 SDD 文档,并创建 ADR 记录决策

#### Scenario: 模块级调整可选更新 SDD
- **WHEN** 变更涉及新增服务层、修改接口定义等模块级调整
- **THEN** 开发者 SHOULD 考虑更新 SDD,建议创建 ADR

#### Scenario: 功能实现无需更新 SDD
- **WHEN** 变更仅实现新功能,不改变现有架构
- **THEN** 开发者无需更新 SDD 或创建 ADR
...
- **决策**: 具体选择了什么方案
- **后果**: 带来的影响和权衡
- **Related Changes**: 相关的 OpenSpec change 名称列表

#### Scenario: 记录新技术选型
- **WHEN** 团队决定采用 OpenSpec 工作流
- **THEN** 应创建 ADR 记录选型理由、替代方案和预期影响,并在 "Related Changes" 中引用相关 change

#### Scenario: ADR 引用 OpenSpec Changes
- **WHEN** 创建新的 ADR
- **THEN** ADR MUST 包含 "Related Changes" 章节,列出所有相关的 OpenSpec change 名称
...
- OpenSpec change 的 `design.md` 如涉及架构决策,应包含 "Related ADRs" 章节

#### Scenario: 架构演进追踪
- **WHEN** 查看某个 OpenSpec change 的 design.md
- **THEN** 应能看到该变更如何符合或扩展现有架构规范,以及相关的 ADR 引用

#### Scenario: Design 中引用 ADR
- **WHEN** 编写涉及架构决策的 OpenSpec change 的 design.md
- **THEN** design.md SHALL 包含 "Related ADRs" 章节,引用受影响的架构决策记录
