...
### 2.2 AI 适配服务 (Rust) (VD-007 ~ VD-011)

| 任务 ID  | 任务名称                     | 优先级 | 估时 | 状态     | 验收标准                         |
| -------- | ---------------------------- | ------ | ---- | -------- | -------------------------------- |
| VD-007   | 创建 AI Provider Trait 定义  | P0     | 3h   | 📋 待开始 | Trait 定义完整，包含所有必要方法 |
| VD-008   | 实现 OpenAI 适配器           | P0     | 6h   | 📋 待开始 | 能调用 OpenAI API 并返回结果     |
| VD-009   | 实现 Kimi 适配器             | P0     | 6h   | 📋 待开始 | 能调用 Kimi API 并返回结果       |
| VD-010   | 实现 AI 服务管理器 (统一入口) | P0     | 4h   | 📋 待开始 | 支持切换厂商，故障转移           |
| VD-011   | 实现流式输出 (SSE) 支持        | P0     | 6h   | 📋 待开始 | AI 响应能实时流式显示            |

### 2.3 核心功能：PRD 与用户画像 (VD-012 ~ VD-018)

| 任务 ID  | 任务名称                    | 优先级 | 估时 | 状态     | 验收标准                     |
| -------- | --------------------------- | ------ | ---- | -------- | ---------------------------- |
| VD-012   | 创建想法输入界面            | P0     | 4h   | 📋 待开始 | 多行文本输入，支持占位提示   |
| VD-013   | 创建 PRD 生成提示词模板     | P0     | 4h   | 📋 待开始 | 模板包含所有必要章节         |
| VD-014   | 实现 PRD 生成 API           | P0     | 4h   | 📋 待开始 | 能调用 AI 生成结构化 PRD     |
| VD-015   | 创建 PRD 展示组件           | P0     | 6h   | 📋 待开始 | 能渲染 Markdown 格式 PRD     |
| VD-016   | 创建用户画像提示词模板      | P0     | 3h   | 📋 待开始 | 模板包含基本信息、需求等模块 |
| VD-017   | 实现用户画像生成 API        | P0     | 3h   | 📋 待开始 | 能生成 3-5 个用户画像          |
| VD-018   | 创建用户画像展示组件        | P0     | 4h   | 📋 待开始 | 卡片式显示，支持切换         |

### 2.4 竞品分析与流程整合 (VD-019 ~ VD-022)

| 任务 ID  | 任务名称               | 优先级 | 估时 | 状态     | 验收标准             |
| -------- | ---------------------- | ------ | ---- | -------- | -------------------- |
| VD-019   | 创建竞品分析提示词模板 | P0     | 3h   | 📋 待开始 | 模板包含竞品对比维度 |
| VD-020   | 实现竞品分析生成 API   | P0     | 3h   | 📋 待开始 | 能生成竞品分析报告   |
| VD-021   | 创建竞品分析展示组件   | P0     | 4h   | 📋 待开始 | 对比表格或卡片展示   |
| VD-022   | 实现 Vibe Design 步骤向导 | P0     | 6h   | 📋 待开始 | 想法→PRD→画像→竞品流程完整 |
...
| VM-003   | 创建营销文案提示词模板 | P1     | 3h   | 📋 待开始 | 支持 Twitter/小红书等平台      |
| VM-004   | 实现营销文案生成 API   | P1     | 3h   | 📋 待开始 | 能生成多平台文案               |
| VM-005   | 创建营销文案展示组件   | P1     | 4h   | 📋 待开始 | 支持复制和导出文案             |
...
├── INFRA-006 ~ INFRA-010 (数据库/密钥)
├── INFRA-011, INFRA-012 (工具检测)
└── INFRA-013, INFRA-014 (Agent 基础框架)
    │
    ▼
Week 2: AI 配置 + Agent 通信
├── VD-001 ~ VD-006 (AI 配置)
├── VD-007 ~ VD-011 (AI 适配器)
├── VC-001 ~ VC-005 (Agent 通信协议)
└── VC-006 ~ VC-011 (Initializer Agent)
    │
    ▼
Week 3: Initializer + HITL
├── VC-006 ~ VC-011 (Initializer 完成)
├── VC-023 ~ VC-028 (HITL 检查点 UI)
└── VC-012 ~ VC-017 (Coding Agent 基础)
    │
    ▼
Week 4: Coding Agents + 质量门禁
├── VC-012 ~ VC-017 (Coding Agent 集群)
├── VC-018 ~ VC-022 (质量门禁 QG-001~QG-003)
└── VC-033 ~ VC-036 (监控与日志)
    │
    ▼
Week 5: MR Creation + 整合
├── VC-029 ~ VC-032 (MR Creation Agent)
├── VC-027 (CP-008 最终审查)
└── VM-001 ~ VM-005 (Marketing)
    │
    ▼
Week 6: 测试与发布
├── 端到端测试
...
| 模块 | 任务 ID 范围 | 任务数 | 占比 |
|------|-------------|--------|------|
| **INFRA** - 基础设施 | INFRA-001 ~ INFRA-014 | 14 | 18% |
| **VD** - Vibe Design | VD-001 ~ VD-022 | 22 | 28% |
| **VC** - Vibe Coding | VC-001 ~ VC-036 | 36 | 46% |
| **VM** - Vibe Marketing | VM-001 ~ VM-005 | 5 | 6% |
| **总计** | | **77** | **100%** |

**注意**: 实际核心任务数为 60 个，部分子任务在详细分解时计数。

### Vibe Design 任务分组

| 分组 | 任务 ID 范围 | 数量 | 说明 |
|------|-------------|------|------|
| AI厂商配置 | VD-001 ~ VD-006 | 6 | API密钥管理与配置 |
| AI适配服务 | VD-007 ~ VD-011 | 5 | Rust后端适配器 |
| 核心功能 | VD-012 ~ VD-022 | 11 | PRD/画像/竞品生成 |

### Vibe Coding 任务分组

| 分组 | 任务 ID 范围 | 数量 | 说明 |
|------|-------------|------|------|
| Agent基础架构 | VC-001 ~ VC-005 | 5 | 通信协议与管理 |
| Initializer Agent | VC-006 ~ VC-011 | 6 | 环境初始化与任务分解 |
| Coding Agent集群 | VC-012 ~ VC-017 | 6 | 并发编码 Agent |
| 质量门禁系统 | VC-018 ~ VC-022 | 5 | 代码检查与回滚 |
| HITL检查点 | VC-023 ~ VC-028 | 6 | 人工审查界面 |
| MR Creation | VC-029 ~ VC-032 | 4 | 合并请求创建 |
| 监控与日志 | VC-033 ~ VC-036 | 4 | 实时监控面板 |
