...
```
graph LR
A[1.任务选择] --> B[2.创建执行计划] --> C[3.架构学习] --> D[4.测试设计] --> E[5.开发实施] --> F[6.质量验证] --> G[7.更新任务状态] --> H[8.任务完成归档] --> I[9.Git 提交]
```
...
[`创建执行计划步骤`](./exec-plans/index.md#场景-1-创建新的执行计划)
...
[`任务完成归档步骤`](./exec-plans/index.md#场景-2-任务完成归档)   
