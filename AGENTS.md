...
本文档为 AI Agent 提供系统结构、可用工具和关键约束的完整导航信息。
...
```
📦 项目根目录
├── 📄 AGENTS.md              # 你正在查看的导航地图
├── 📄 package.json           # 依赖和脚本配置
├── 📄 vite.config.ts         # 前端构建配置
├── 📄 tauri.conf.json        # Tauri 后端配置 (位于 src-tauri/)
│
...
### 决策记录 (ADRs)
位置：`.harness/context-engineering/decision-records/`

已记录的架构决策：
- [ADR-001](./harness/context-engineering/decision-records/adr-001-typescript-strict-mode.md) - 启用 TypeScript 严格模式
- [ADR-002](./harness/context-engineering/decision-records/adr-002-zustand-state-management.md) - 使用 Zustand 进行状态管理
- [ADR-003](./harness/context-engineering/decision-records/adr-003-tauri-v2-architecture.md) - Tauri v2 前后端分离架构

### 执行日志
位置：`.harness/context-engineering/execution-logs/`

记录重要操作的执行结果和调试信息。

### 最佳实践
位置：`.harness/context-engineering/knowledge-base/best-practices.md`
...
### 新增功能流程
1. **创建决策记录**: 在 `decision-records/` 中记录设计思路
2. **实现代码**: 遵循架构约束和代码规范
3. **更新文档**: 同步更新相关文档和注释
